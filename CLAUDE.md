...
가상 현금 1억원으로 시작해 코스피/코스닥 종목을 자동 매매하고 실적을 공유하는 기능. **구현 진행 중 — 설계 문서는 [PRD.md](./PRD.md)에 전체가 있다.** `src/portfolio.py`(원장), `src/trading_agent.py`(규칙 엔진+가드레일), `scripts/run_daily_trading.py`(①~④를 묶는 단일 진입점), `pages/모의투자.py`+`demo_app.py`(리포팅 UI), `.github/workflows/daily_trading.yml`+`requirements-trading.txt`(스케줄러)까지 코드는 완료(PRD.md 10장 진행 상태 참고). **단, 스케줄러는 `workflow_dispatch`만 켜져 있고 `schedule`은 주석 처리 상태 — 실제 GitHub 저장소에서 수동 실행으로 커밋·푸시를 눈으로 확인한 뒤 주석을 해제해야 자동으로 돈다(로컬에서 검증 불가능한 항목).** 여기는 앞으로 이 기능을 만들거나 건드릴 때 CLAUDE.md 규칙에 편입되는 요약만 적는다. 자세한 근거·수치·프롬프트 스키마는 PRD.md를 본다.
